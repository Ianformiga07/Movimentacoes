  ...
  $(document.body).on("click", ".jq-paginacao", function () {
    ...
    var codProp = $("[name='codProp']").val();
    $.ajax({
        type: "POST",
        url: "http://localhost/MovimentaGTA/ajax/tab_buscaMovimenta.ajax.asp",
        data: 'Pagina=' + Pagina + '&cod=' + cod + '&codProp=' + codProp ,
        cache: false,
        beforeSend: function () {
            showLoader();
        },
        ...
        success: function (response) {
            ...
            setTimeout((function () {
                ...
                if(response) {
                    $(".alvo-buscaMovimeta").html(response);
                }
            }), 300);
        }
    });
